...
std::expected<void, InlineHook::Error> InlineHook::e9_hook(const std::shared_ptr<Allocator>& allocator) {
    ...
    for (auto ip = m_target, tramp_ip = m_trampoline.data(); ip < m_target + m_original_bytes.size(); ip += ix.length) {
        ...
        if (ip[0] == 0xE8) {
            ...
            // Handle thunks generated by -fPIC
            // __x86_get_pc_thunk_ax/bx/cx/dx/si/di/bp
            if (thunk[0] == 0x8B && thunk[2] == 0x24 && thunk[3] == 0xC3) {
                ...
                switch (thunk[1]) {
                    ...
                    break;
                    default:
                    return std::unexpected{Error::unsupported_instruction_in_trampoline(thunk)};
                }
                // Write the value the register should have, directly into the register
                std::uint8_t opcodes[5];
                opcodes[0] = mov;
